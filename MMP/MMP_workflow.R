...
for(i in 1:nrow(Summary_list)){
  ...
  frame = Summary_list[(Unified_IDs==Sample_ID)&!is.na(Summary_list$MMP),]
  if(nrow(frame)==0){
    next
  }
  Nr_of_samples = nrow(Summary_list[(Unified_IDs==Sample_ID)&!is.na(Summary_list$MMP),])
  ...
  for(j in 1:nrow(workframe)){
    ...
    MMP_file = Rawdata[grepl("Image", Rawdata)]
    MMPcsv = read.csv(MMP_file, header = TRUE)
    
    MMP = as.data.frame(matrix(data = MMPcsv$Mean_Cytoplasm_Math_RedGreenRatio, nrow = 16, byrow = T))
    rownames(MMP) = c("A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P")
    
    Confluency = as.data.frame(matrix(data = MMPcsv$Math_CellConfluency, nrow = 16, byrow = T))
    rownames(Confluency) = c("A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P")
    
    #Mitochondrial membrane potential (MMP)
    MMPUnexposed_mean = mean(outliers::rm.outlier(as.numeric(unlist(MMP[,23:24]))),na.rm=T)
    DecreaseinMMP = 100-(MMP/MMPUnexposed_mean*100)
    workframe$Effect[j] = paste0(DecreaseinMMP[Row,1:22],collapse = ";")
    workframe$Effect_Control = paste0(unlist(c(DecreaseinMMP[,23:24])),collapse = ";")
    
    #Viability using images from ImageXpress
    Unexposed_mean = mean(outliers::rm.outlier(as.numeric(unlist(Confluency[,23:24]))),na.rm=T)
    Cytotoxicity = 100-(Confluency/Unexposed_mean*100)
  ...
  if(combine_repeats==F&!grepl(Reference,Sample_ID)){
    ...
    indices = indices[!is.na(Summary_list$MMP[indices])]
  ...
  cytotoxicity_linear_split = cytotoxicity_linear_split<(plyr::round_any(quantile(Cytotoxicity_control,probs = c(0.01),na.rm = T),1,f=ceiling))
  for(c in 1:ncol(cytotoxicity_linear_split)){
    if(!is.na(match(T,cytotoxicity_linear_split[,c]))&match(T,cytotoxicity_linear_split[,c])<=4&(length(cytotoxicity_linear_split[cytotoxicity_linear_split==T])!=length(cytotoxicity_linear_split))){
  ...
  Cytotoxicity_linear[Cytotoxicity_linear>linear_cutoff_effect] = NA
  ...
  #use the 1 % quantile of the control as threshold, but round up to 1% (outliers could be problematic)
  cytotoxicity_split = cytotoxicity_split<(plyr::round_any(quantile(Cytotoxicity_control,probs = c(0.01),na.rm = T),1,f=ceiling))
  for(c in 1:ncol(cytotoxicity_split)){
    if(!is.na(match(T,cytotoxicity_split[,c]))&match(T,cytotoxicity_split[,c])<=4&(length(cytotoxicity_split[cytotoxicity_split==T])!=length(cytotoxicity_split))){
  ...
  if(final_model_cyt=="cnst"){
    ...
    loglogistic_regression_cytotoxicity = tcpl::tcplFit(logc=frame$x,resp=frame$y,bmad=bmad,force.fit = T)
    Differences = abs(frame$y-loglogistic_regression_cytotoxicity$gnls_modl)
  ...
  if(is.nan(IC10_log)|!is.numeric(IC10_log)|10^(max(frame$x,na.rm=T))<IC10_log){
    IC10_log = "no cytotoxicity"
    SE_IC10_log = ""
    R2_IC10_log = ""
  } else {
  ...
  #Effects
  Results_frame$MMP_Effect_dev_Control = Effect_dev_Control
  Results_frame$MMP_linmodel_EC10 = EC10_linear
  Results_frame$SE_MMP_linmodel_EC10 = SE_EC10_linear
  Results_frame$R2_MMP_linmodel_EC10 = R2_EC10_linear
  if(!is.na(as.numeric(EC10_linear))&!is.na(as.numeric(SE_EC10_linear))){
    Results_frame$MMP_linmodel_confidence_EC10 = plyr::round_any((1-abs(as.numeric(SE_EC10_linear/as.numeric(EC10_linear))))*confidence_factor_effect_linear*R2_EC10_linear,0.01,ceiling)
  } else {
    Results_frame$MMP_linmodel_confidence_EC10 = NA
  }
  Results_frame$MMP_logmodel_EC10 = EC10_log
  Results_frame$SE_MMP_logmodel_EC10 = SE_EC10_log
  Results_frame$R2_MMP_logmodel_EC10 = R2_EC10_log
  if(!is.na(as.numeric(EC10_log))&!is.na(as.numeric(SE_EC10_log))){
    Results_frame$MMP_logmodel_confidence_EC10 = plyr::round_any((1-abs(as.numeric(SE_EC10_log/as.numeric(EC10_log))))*confidence_factor_effect_log*R2_EC10_log,0.01,ceiling)
  } else {
    Results_frame$MMP_logmodel_confidence_EC10 = NA
  }
  Results_frame$MMP_logmodel_linear_EC10 = 10^(as.numeric(EC10_log))
  
  #Cytotoxicity
  Results_frame$MMP_Cytotoxicity_dev_Control = Cytotoxicity_dev_Control
  Results_frame$MMP_linmodel_IC10 = IC10_linear
  Results_frame$SE_MMP_linmodel_IC10 = SE_IC10_linear
  Results_frame$R2_MMP_linmodel_IC10 = R2_IC10_linear
  if(!is.na(as.numeric(IC10_linear))&!is.na(as.numeric(SE_IC10_linear))){
    Results_frame$MMP_linmodel_confidence_IC10 = plyr::round_any((1-abs(as.numeric(SE_IC10_linear/as.numeric(IC10_linear))))*confidence_factor_cyt_linear*R2_IC10_linear,0.01,ceiling)
  } else {
    Results_frame$MMP_linmodel_confidence_IC10 = NA
  }
  Results_frame$MMP_logmodel_IC10 = IC10_log
  Results_frame$SE_MMP_logmodel_IC10 = SE_IC10_log
  Results_frame$R2_MMP_logmodel_IC10 = R2_IC10_log
  Results_frame$MMP_logmodel_linear_IC10 = 10^(as.numeric(IC10_log))
  if(!is.na(as.numeric(IC10_log))&!is.na(as.numeric(SE_IC10_log))){
    Results_frame$MMP_logmodel_confidence_IC10 = plyr::round_any((1-abs(as.numeric(SE_IC10_log/as.numeric(IC10_log))))*confidence_factor_cyt_log*R2_IC10_log,0.01,ceiling)
  } else {
    Results_frame$MMP_logmodel_confidence_IC10 = NA
  }
  ...
  colnames(Linear_all) = c(paste0("Conc (",workframe$Unit[1],")"),create_names(workframe))
  ...
  #Visualization
  Effect_color = rgb(235/255,191/255,50/255,1)
  ...
  p_right = ggplot(Linear_all_frame,aes(x=Concentration)) + 
    ...
    scale_y_continuous(
      name = "Decrease in MMP [%]", limits = c(min_y_axis_limits_effects,log_y_axis_limits_effects),position="right")
  ...
  p_right = ggplot(Linear_effect_frame,aes(x=x)) + 
    ...
    scale_y_continuous(
      name = "Decrease in MMP [%]",position = "right") + coord_cartesian(ylim=c(min_y_axis_limits_effects,linear_y_axis_limits_effects),clip = "on")
  ...
  p_right = ggplot(Log_effect_frame,aes(x=x)) + 
    ...
    scale_y_continuous(
      name = "Decrease in MMP [%]",position = "right") + coord_cartesian(ylim=c(min_y_axis_limits_effects,log_y_axis_limits_effects),clip = "on")
...
if(Summary_model_effects=="linear"){
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$MMP_linmodel_EC10[i]))){
      if(grepl("masked",Summary_results$MMP_linmodel_EC10[i])){
    ...
    } else if(is.na(as.numeric(Summary_results$MMP_linmodel_confidence_EC10[i]))|as.numeric(Summary_results$MMP_linmodel_confidence_EC10[i])<0.70){
      Final_Summary_Effects[i,3] = Summary_results$MMP_logmodel_EC10[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_MMP_logmodel_EC10[i]
      Final_Summary_Effects[i,5] = FALSE
    } else {
      Final_Summary_Effects[i,3] = Summary_results$MMP_linmodel_EC10[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_MMP_linmodel_EC10[i]
  ...
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$MMP_logmodel_EC10[i]))){
      if(grepl("masked",Summary_results$MMP_logmodel_EC10[i])){
        Final_Summary_Effects[i,3] = "masked by cytotoxicity"
        Final_Summary_Effects[i,4] = NA
        Final_Summary_Effects[i,5] = FALSE
      } else {
    ...
    } else if(is.na(as.numeric(Summary_results$MMP_logmodel_confidence_EC10[i]))|as.numeric(Summary_results$MMP_logmodel_confidence_EC10[i])<0.70){
      Final_Summary_Effects[i,3] = Summary_results$MMP_logmodel_EC10[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_MMP_logmodel_EC10[i]
      Final_Summary_Effects[i,5] = FALSE
    } else {
...
if(Summary_model_cytotoxicity=="linear"){
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$MMP_linmodel_IC10[i]))){
      Final_Summary_Cytotoxicity[i,1] = "no cytotoxicity"
      Final_Summary_Cytotoxicity[i,2] = NA
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else if(is.na(as.numeric(Summary_results$MMP_linmodel_confidence_IC10[i]))|as.numeric(Summary_results$MMP_linmodel_confidence_IC10[i])<0.70){
      Final_Summary_Cytotoxicity[i,1] = Summary_results$MMP_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_MMP_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else {
      Final_Summary_Cytotoxicity[i,1] = Summary_results$MMP_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_MMP_linmodel_IC10[i]+
  ...
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$MMP_logmodel_IC10[i]))){
      Final_Summary_Cytotoxicity[i,1] = "no cytotoxicity"
      Final_Summary_Cytotoxicity[i,2] = NA
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else if(is.na(as.numeric(Summary_results$MMP_logmodel_confidence_IC10[i]))|as.numeric(Summary_results$MMP_logmodel_confidence_IC10[i])<0.70){
      Final_Summary_Cytotoxicity[i,1] = Summary_results$MMP_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_MMP_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else {
      Final_Summary_Cytotoxicity[i,1] = Summary_results$MMP_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_MMP_logmodel_IC10[i]
...
openxlsx::saveWorkbook(Output,paste0(Output_path,"/Summary_",project,"_MMP_filled.xlsx"),overwrite = T)
