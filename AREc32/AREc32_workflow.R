list.of.packages <- c("dplyr","EnvStats","stringr","gtools","ggplot2","ggprism","ggpubr","yaml","patchwork","tcpl","drc")
...
Summary_path = Summary_path[!grepl(paste0("filled|~",assays),Summary_path)]
...
for(i in 1:nrow(Summary_list)){
  if(is.na(Summary_list$AREc32[i])){
    next
  }
  ...
  frame = Summary_list[(Unified_IDs==Sample_ID)&!is.na(Summary_list$AREc32),]
  if(nrow(frame)==0){
    next
  }
  Nr_of_samples = nrow(Summary_list[(Unified_IDs==Sample_ID)&!is.na(Summary_list$AREc32),])
  ...
  for(j in 1:nrow(workframe)){
    ...
    RLU_file = Rawdata[grepl("RawData",Rawdata)]
    t24_file = Rawdata[grepl("t24",Rawdata)]
    t48_file = Rawdata[grepl("t48",Rawdata)]
    RLU = openxlsx::read.xlsx(RLU_file,rows=seq(23,39,1),cols=seq(2,25,1))
    rownames(RLU) = c("A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P")
    t24 = as.data.frame(matrix(as.numeric(as.matrix(read.table(t24_file,dec = ",",skip = 9))),nrow = 16,ncol=24))
    t48 = as.data.frame(matrix(as.numeric(as.matrix(read.table(t48_file,dec = ",",skip = 9))),nrow = 16,ncol=24))
    
    #Receptor Activation
    #Unexposed_mean = mean(Rosner_outlier_removal(unlist(c(Neurite[,23:24]))),na.rm = T)
    RLU_min = mean(outliers::rm.outlier(as.numeric(unlist(RLU[,23:24]))),na.rm=T)
    Effect = RLU/RLU_min #Induction Ratio
    workframe$Effect[j] = paste0(Effect[Row,1:22],collapse = ";")
    workframe$Effect_Control[j] = paste0(unlist(c(Effect[,23:24])),collapse = ";")
    
    #Viability
    #Values = 100*(1-Red/Green)
    #Unexposed_mean = mean(Rosner_outlier_removal(unlist(c(Values[,23:24]))),na.rm = T)
    Unexposed_mean = mean(outliers::rm.outlier(as.numeric(unlist(t48[,23:24]))),na.rm=T)
    Cytotoxicity = 100-(t48/Unexposed_mean*100)
    workframe$Cytotox[j] = paste0(Cytotoxicity[Row,1:22],collapse = ";")
    workframe$Cytotox_Control[j] = paste0(unlist(c(Cytotoxicity[,23:24])),collapse = ";")
  }
  
  #-----------------------------------------------------------------------------
  if(combine_repeats==F&!grepl(Reference,Sample_ID)){
    indices = which(Unified_IDs==Sample_ID)
    indices = indices[!is.na(Summary_list$AREc32[indices])]
  ...
  #Fit data from workframe, create plots, set up input for Prism, and calculate ECIR1.5 and IC10 
  ...
  Linear_effect_control_frame = data.frame("x"=10^log_concentration_unexposed_controls,
                                        "y"=Effect_control,
                                        "Name"=rep(workframe$Measurement,each=32))
  Linear_effect_control_frame = Linear_effect_control_frame[Linear_effect_control_frame$y<linear_cutoff_cytotoxicity,]
  ...
  #-----------------------------------------------------------------------------
  #Cytotoxicity
  #Test if Effect is different from control
  t_test = t.test(Cytotoxicity,Cytotoxicity_control,alternative = "greater")
  ...
  #Linear Regression
  #Prepare filter for effect values
  ...
  #Remove values above linear_cutoff_cytotoxicity %
  ...
  if(!is.na(linear_regression_cytotox)){
    ...
    #Cytotoxicity_linear[Cytotoxicity_linear%in%influentials] = NA
    ...
    SE_slope = sqrt(diag(vcov(linear_regression_cytotox)))
  ...
  if(length(frame_plateau$y[frame_plateau$y>10])>2){
    ...
    model = tryCatch(drc::drm(y~x,data=frame_plateau,fct=drc::MM.3()),
    ...
    if(!is.na(model)){
      ...
      while(MM_AIC<(Linear_AIC-4)&length(frame_plateau$y)>4){
        ...
        model_linear = lm(y~0+x,data=frame_plateau)
        predi = predict(model,newdata = frame_plateau)
        predi_linear = predict(model_linear,newdata=frame_plateau)
        plot(frame_plateau$y~frame_plateau$x,type="p")
        lines(frame_plateau$x,predi,col="red")
        lines(frame_plateau$x,predi_linear,col="blue")
        MM_AIC = round(as.numeric(2*3-2*(logLik(model))),digits = 0)
  ...
  Log_cyt_frame = Log_cyt_frame[(!is.na(Log_cyt_frame$y)&Log_cyt_frame$y<=110),]
  Cytotoxicity[is.na(frame$y)&frame$y>110] = NA
  order_coordinates = order(frame$x,decreasing = T)
  frame = frame[(!is.na(frame$y)&frame$y<=110),]
  ...
  bmad = sd(Cytotoxicity_control,na.rm = T)
  ...
  if(is.na(IC10_log)|is.nan(IC10_log)|!is.numeric(IC10_log)|10^(max(frame$x,na.rm=T))<IC10_log){
    IC10_log = "no cytotoxicity"
    SE_IC10_log = ""
    R2_IC10_log = ""
    confidence_factor_cyt_log = 0.5
    ...
    #Combine repeats
    replicates = as.data.frame(table(frame$x))[1,2]
    dose_new = 10^(frame$x[seq(1,length(frame$x),by=replicates)])
  ...
  if(nrow(frame)>5){
    linear_regression_effect = tryCatch(lm(data=frame,I(y-1)~0+x),error=function(e){linear_regression_effect=NA})
    influentials = c()
    if(!is.na(linear_regression_effect)){
      ...
      if(r2 < 0.8 & length(Concis)>5){
        ...
        while(redo_calib == T){
          ...
          if(!is.nan(influential)){
            ...
            Filter_frame = !is.nan(frame_new$y)
            frame_new = frame_new[!is.nan(frame_new$y),]
            model_new = lm(formula=I(y-1)~0+x,data = frame_new)
            r2 = round(summary(model_new)$adj.r.squared,digits = 2)
            if(r2<r2_initial){
              redo_calib = F
              r2 = r2_initial
      ...
      SE_slope = sqrt(diag(vcov(linear_regression_effect)))
      
      plot(frame$y~frame$x,type="p")
      predi = predict(linear_regression_effect,new.data=frame)+1
      lines(frame$x,predi,col="red")
    ...
    if(!is.na(linear_regression_effect)){
      ...
      if(length(frame_plateau$y[frame_plateau$y>1.5])>2){
        frame_plateau = frame_plateau[order(frame_plateau$x,decreasing = F),]
        frame_plateau$x = as.numeric(as.character(frame_plateau$x))
        model = tryCatch(drc::drm(y~x,data=frame_plateau,fct=drc::MM.2()),
                         error=function(e){model=NA})
        if(!is.na(model)){
          model_linear = lm(I(y-1)~0+x,data=frame_plateau)
          predi = predict(model,newdata = frame_plateau)
          predi_linear = predict(model_linear,newdata=frame_plateau)+1
          plot(frame_plateau$y~frame_plateau$x,type="p")
          lines(frame_plateau$x,predi,col="red")
          lines(frame_plateau$x,predi_linear,col="blue")
          MM_AIC = round(as.numeric(2*3-2*(logLik(model))),digits = 0)
      ...
      linear_regression_effect = tryCatch(lm(data=frame,I(y-1)~0+x,weights = x^2),error=function(e){linear_regression_effect=NA})
      influentials = c()
      if(!is.na(linear_regression_effect)){
        r2 = round(summary(linear_regression_effect)$adj.r.squared,digits = 2)
        Concis = frame[!is.na(frame[,2]),1]
      ...
      if(is.na(linear_regression_effect)|max(frame$x,na.rm=T)<(0.5/slope)|slope<0){
        ECIR1.5_linear = "no effect"
        SE_ECIR1.5_linear = ""
        R2_ECIR1.5_linear = ""
        confidence_factor_effect_linear = 0.5
      } else {
        ECIR1.5_linear = 0.5/slope
        SE_ECIR1.5_linear = 0.5/slope^2*SE_slope
        R2_ECIR1.5_linear = r2
        
        #Combine repeats
        replicates = as.data.frame(table(frame$x))[1,2]
        dose_new = frame$x[seq(1,length(frame$x),by=replicates)]
        
        if(length(dose_new[dose_new>ECIR1.5_linear])<2){
          confidence_factor_effect_linear = 0.5
        } else {
        ...
        if(cytotoxicity_cutoff==T){
          if(is.numeric(IC10_linear)&IC10_linear<ECIR1.5_linear){
            ECIR1.5_linear = "masked by cytotoxicity"
            SE_ECIR1.5_linear = ""
            R2_ECIR1.5_linear = ""
            confidence_factor_effect_linear = 0.5
          }
  ...
  if(nrow(frame)>5){
    Log_effect_frame = Log_effect_frame[!is.na(Log_effect_frame$y),]
    Effect[is.na(frame$y)] = NA
    frame = frame[!is.na(frame$y),]
    ...
    bmad = sd(Effect_control,na.rm = T)
    #loglogistic_regression_Effect = tcpl::tcplFit(logc=frame$x,resp=frame$y,bmad=bmad,force.fit = T)
    
    #Use drc package since AREc32 with minimum of 1 is not suitable for tcpl
    if(nrow(frame)>3){
      tryCatch({loglogistic_regression_Effect = drc::drm(y~x, data=frame, fct=LL.4(fixed = c(NA,1,NA,NA),names = c("Slope", "Lower Limit", "Upper Limit", "AC50")), logDose = 10, control = drmc(errorm = F))},
               error=function(e){loglogistic_regression_Effect=NA})
      tryCatch({loglogistic_regression_Effect_constant=drc::drm(y~x, data=frame, fct=LL.4(fixed = c(0,1,1,NA),names = c("Slope", "Lower Limit", "Upper Limit", "AC50")), logDose = 10)},
               error=function(e){loglogistic_regression_Effect_constant=NA})
      if(is.na(loglogistic_regression_Effect)|(length(loglogistic_regression_Effect)<length(loglogistic_regression_Effect_constant))|max(frame$y,na.rm=T)<1.5){
        final_model_effect = "cnst"
      } else {
        LM = as.numeric(logLik(loglogistic_regression_Effect)) #4 model parameters
        L0 = as.numeric(logLik(loglogistic_regression_Effect_constant)) #1 model parameter
        
        AIC_LM = -2*(LM) + 2*4 + (2*4*(4+1)/(nrow(Log_effect_frame)-4-1))
        AIC_L0 = -2*(L0) + 2*1 + (2*1*(1+1)/(nrow(Log_effect_frame)-1-1))
        
        AICs = c(AIC_LM,AIC_L0)
        model_names = c("hill","cnst")
    ...
    if(final_model_effect=="cnst"){
      ...
      if(length(Differences)>=5){
        ...
        loglogistic_regression_Effect = drc::drm(y~x, data=frame, fct=LL.4(fixed = c(NA,1,NA,NA),names = c("Slope", "Lower Limit", "Upper Limit", "AC50")), logDose = 10)
        frame_relative = frame
        frame_relative$y = ((frame_relative$y - min(frame_relative$y,na.rm=T)) / (max(frame_relative$y,na.rm=T) - min(frame_relative$y,na.rm=T))) * 100
        loglogistic_regression_Effect_tcpl = tcpl::tcplFit(logc=frame_relative$x,resp=frame_relative$y,bmad=bmad,force.fit = T)
        transformation_frame = data.frame("x"=frame$y,"y"=frame_relative$y)
        transformation = lm(y~x,data=transformation_frame)
        transformi = data.frame("x"=1.5)
        needed_percent = as.numeric(predict(transformation,newdata=transformi))
        Top = loglogistic_regression_Effect_tcpl$hill_tp
        Bottom = 0
        log_Slope = loglogistic_regression_Effect_tcpl$hill_gw
        EC50_log = loglogistic_regression_Effect_tcpl$hill_ga
        SEM_Slope = loglogistic_regression_Effect_tcpl$hill_gw_sd/sqrt(nrow(frame))
        SEM_EC50 = loglogistic_regression_Effect_tcpl$hill_ga_sd/sqrt(nrow(frame))
        SEM_ECIR1.5 = calculate_SE_ac10(Top,Bottom,needed_percent,EC50_log,SEM_EC50,log_Slope,SEM_Slope)
        
        if(is.nan(SEM_ECIR1.5)){
          SEM_ECIR1.5 = "no error calculation possible"
        }
        
        L0 = exp((AICs[2]-(2*1 + (2*1*(1+1)/(nrow(frame)-1-1))))/(-2))
        Lm = exp((AICs[1]-(2*4 + (2*4*(4+1)/(nrow(frame)-4-1))))/(-2))
        ...
        predictions = predict(loglogistic_regression_Effect,newdata = frame)
        plot(frame$y~frame$x,type="p")
        lines(frame$x,predictions,type="l",col="red")
        
        ECIR1.5_log = tcpl::tcplHillACXX(needed_percent,tp=Top,ga=EC50_log,gw=log_Slope)
        
        abline(v=ECIR1.5_log)
        abline(h=1.5)
      ...
      predictions = predict(loglogistic_regression_Effect,newdata = frame)
      Differences = abs(frame$y-predictions)
      if(length(Differences)>=5){
        Differences = Rosner_outlier_removal(Differences)
        frame = frame[!is.na(Differences),]
        Effect[is.na(Differences)] = NA
    ...
    if(is.nan(ECIR1.5_log)|!is.numeric(ECIR1.5_log)|10^(max(frame$x,na.rm=T))<ECIR1.5_log){
      ECIR1.5_log = "no effect"
      SE_ECIR1.5_log = ""
      R2_ECIR1.5_log = ""
    } else {
      ECIR1.5_log = ECIR1.5_log
      SE_ECIR1.5_log = SEM_ECIR1.5
      R2_ECIR1.5_log = round(corrected_CoxSnellR2,digits = 2)
      
      #Combine repeats
      replicates = as.data.frame(table(frame$x))[1,2]
      dose_new = 10^(frame$x[seq(1,length(frame$x),by=replicates)])
      
      if(length(dose_new[dose_new>ECIR1.5_log])<2){
        confidence_factor_effect_log = 0.5
      } else {
      ...
      if(cytotoxicity_cutoff==T){
        if(is.numeric(IC10_log)&IC10_log<ECIR1.5_log){
          ECIR1.5_log = "masked by cytotoxicity"
          SE_ECIR1.5_log = ""
          R2_ECIR1.5_log = ""
          confidence_factor_effect_log = 0.5
        }
  ...
  #Effects
  Results_frame$AREc32_Effect_dev_Control = Effect_dev_Control
  Results_frame$AREc32_linmodel_ECIR1.5 = ECIR1.5_linear
  Results_frame$SE_AREc32_linmodel_ECIR1.5 = SE_ECIR1.5_linear
  Results_frame$R2_AREc32_linmodel_ECIR1.5 = R2_ECIR1.5_linear
  if(!is.na(as.numeric(ECIR1.5_linear))&!is.na(as.numeric(SE_ECIR1.5_linear))){
    Results_frame$AREc32_linmodel_confidence_ECIR1.5 = plyr::round_any((1-abs(as.numeric(SE_ECIR1.5_linear/as.numeric(ECIR1.5_linear))))*confidence_factor_effect_linear*R2_ECIR1.5_linear,0.01,ceiling)
  } else {
    Results_frame$AREc32_linmodel_confidence_ECIR1.5 = NA
  }
  Results_frame$AREc32_logmodel_ECIR1.5 = ECIR1.5_log
  Results_frame$SE_AREc32_logmodel_ECIR1.5 = SE_ECIR1.5_log
  Results_frame$R2_AREc32_logmodel_ECIR1.5 = R2_ECIR1.5_log
  if(!is.na(as.numeric(ECIR1.5_log))&!is.na(as.numeric(SE_ECIR1.5_log))){
    Results_frame$AREc32_logmodel_confidence_ECIR1.5 = plyr::round_any((1-abs(as.numeric(SE_ECIR1.5_log/as.numeric(ECIR1.5_log))))*confidence_factor_effect_log*R2_ECIR1.5_log,0.01,ceiling)
  } else {
    Results_frame$AREc32_logmodel_confidence_ECIR1.5 = NA
  }
  Results_frame$AREc32_logmodel_linear_ECIR1.5 = 10^(as.numeric(ECIR1.5_log))
  
  #Cytotoxicity
  Results_frame$AREc32_Cytotoxicity_dev_Control = Cytotoxicity_dev_Control
  Results_frame$AREc32_linmodel_IC10 = IC10_linear
  Results_frame$SE_AREc32_linmodel_IC10 = SE_IC10_linear
  Results_frame$R2_AREc32_linmodel_IC10 = R2_IC10_linear
  if(!is.na(as.numeric(IC10_linear))&!is.na(as.numeric(SE_IC10_linear))){
    Results_frame$AREc32_linmodel_confidence_IC10 = plyr::round_any((1-abs(as.numeric(SE_IC10_linear/as.numeric(IC10_linear))))*confidence_factor_cyt_linear*R2_IC10_linear,0.01,ceiling)
  } else {
    Results_frame$AREc32_linmodel_confidence_IC10 = NA
  }
  Results_frame$AREc32_logmodel_IC10 = IC10_log
  Results_frame$SE_AREc32_logmodel_IC10 = SE_IC10_log
  Results_frame$R2_AREc32_logmodel_IC10 = R2_IC10_log
  Results_frame$AREc32_logmodel_linear_IC10 = 10^(as.numeric(IC10_log))
  if(!is.na(as.numeric(IC10_log))&!is.na(as.numeric(SE_IC10_log))){
    Results_frame$AREc32_logmodel_confidence_IC10 = plyr::round_any((1-abs(as.numeric(SE_IC10_log/as.numeric(IC10_log))))*confidence_factor_cyt_log*R2_IC10_log,0.01,ceiling)
  } else {
    Results_frame$AREc32_logmodel_confidence_IC10 = NA
  }
  ...
  colnames(Linear_cutoff) = c(paste0("Conc (",workframe$Unit[1],")"),create_names(workframe))
  ...
  colnames(Log_w_excluded) = c(paste0("Conc (",workframe$Unit[1],")"),create_names(workframe))
  ...
  colnames(Controls_log) = c(paste0("logConc (",workframe$Unit[1],")"),create_names(workframe))
  ...
  #Visualization
  Effect_color = rgb(255/255,63/255,6/255,1)
  ...
  p_left = ggplot(Linear_all_control_frame,aes(x=Concentration)) + 
    ...
    geom_point(data=Linear_all_control_frame,aes(y = Effect*12.2222222-12.2222222,shape= Name),fill=Effect_color,size=8) +
    scale_y_continuous(
  ...
  p_right = ggplot(Linear_all_frame,aes(x=Concentration)) + 
    geom_point(data=Linear_all_frame,aes(y = Cytotox*0.08181818182+1,shape = Name),fill=Cyto_color,size=8) + 
    geom_point(data=Linear_all_frame,aes(y = Effect,shape= Name),fill=Effect_color,size=8) +
    scale_y_continuous(
      name = "Nrf2 activation [IR]", limits = c(min_y_axis_limits_effects,log_y_axis_limits_effects),position="right")
  ...
  if(is.na(linear_regression_effect)|nrow(Linear_effect_frame)<3){
    linear_effect_pred = 1
    Linear_effect_frame = data.frame("x"=Concentrations_linear,
                                     "y"=Effect_linear,
                                     "Name"=rep(workframe$Measurement,each=22))
  } else {
    linear_effect_pred = predict(linear_regression_effect,Linear_effect_frame)+1
  }
  ...
  p_left = ggplot(Linear_effect_control_frame,aes(x=x)) + 
    ...
    geom_line(aes(y=1.5),linetype="dashed",color=Effect_color,linewidth=2) + 
    labs(y=NULL) + ylim(min_y_axis_limits_effects,linear_y_axis_limits_effects) + xlim((10^log_concentration_unexposed_controls)*0.99999,(10^log_concentration_unexposed_controls)*1.00001)
  ...
  p_right = ggplot(Linear_effect_frame,aes(x=x)) + 
    ...
    scale_y_continuous(
      name = "Nrf2 activation [IR]",position = "right") + coord_cartesian(ylim=c(min_y_axis_limits_effects,linear_y_axis_limits_effects),clip = "on")
  ...
  p_left = ggplot(Log_all_control_frame,aes(x=Concentration)) + 
    ...
    geom_point(data=Log_all_control_frame,aes(y = Effect*12.2222222-12.2222222,shape= Name),fill=Effect_color,size=8) +
    scale_y_continuous(
  ...
  p_right = ggplot(Log_all_frame,aes(x=Concentration)) + 
    geom_point(data=Log_all_frame,aes(y = Cytotox*0.08181818182+1,shape = Name),fill=Cyto_color,size=8) + 
  ...
  #Fit
  if(is.na(loglogistic_regression_cytotoxicity)|final_model_cyt=="cnst"|is.character(IC10_log)|nrow(Log_cyt_frame)<3){
    log_cyt_pred = 0
    Log_cyt_frame = data.frame("x"=Concentrations_log,
                               "y"=Cytotoxicity,
                               "Name"=rep(workframe$Measurement,each=22))
  ...
  p_left = ggplot(Log_cyt_control_frame,aes(x=x)) + 
    ...
    geom_line(aes(y=10),linetype="dashed",color=Cyto_color,linewidth=2) + 
    labs(y=NULL) + ylim(min_y_axis_limits_cytotoxicity,log_y_axis_limits_cytotoxicity) + xlim((log_concentration_unexposed_controls)*0.99999,(log_concentration_unexposed_controls)*1.00001)
  ...
  if(is.na(loglogistic_regression_Effect)|final_model_effect=="cnst"|is.character(ECIR1.5_log)|nrow(Log_effect_frame)<3){
    log_effect_pred = 1
    Log_effect_frame = data.frame("x"=Concentrations_log,
                                  "y"=Effect,
                                  "Name"=rep(workframe$Measurement,each=22))
  } else {
    log_effect_pred = predict(loglogistic_regression_Effect,newdata = Log_effect_frame)
  }
  ...
  p_left = ggplot(Log_effect_control_frame,aes(x=x)) + 
    ...
    geom_line(aes(y=1.5),linetype="dashed",color=Effect_color,linewidth=2) + 
    labs(y=NULL) + ylim(min_y_axis_limits_effects,log_y_axis_limits_effects) + xlim((log_concentration_unexposed_controls)*0.99999,(log_concentration_unexposed_controls)*1.00001)
  ...
  p_right = ggplot(Log_effect_frame,aes(x=x)) + 
    ...
    scale_y_continuous(
      name = "Nrf2 activation [IR]",position = "right") + coord_cartesian(ylim=c(min_y_axis_limits_effects,log_y_axis_limits_effects),clip = "on")
...
colnames(Final_Summary_Effects) = c("Sample","Unit","ECIR1.5","SE_ECIR1.5","Confident_ECIR1.5?")
...
if(Summary_model_effects=="linear"){
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$AREc32_linmodel_ECIR1.5[i]))){
      if(grepl("masked",Summary_results$AREc32_linmodel_ECIR1.5[i])){
    ...
    } else if(is.na(as.numeric(Summary_results$AREc32_linmodel_confidence_ECIR1.5[i]))|as.numeric(Summary_results$AREc32_linmodel_confidence_ECIR1.5[i])<0.70){
      Final_Summary_Effects[i,3] = Summary_results$AREc32_logmodel_ECIR1.5[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_AREc32_logmodel_ECIR1.5[i]
      Final_Summary_Effects[i,5] = FALSE
    } else {
      Final_Summary_Effects[i,3] = Summary_results$AREc32_linmodel_ECIR1.5[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_AREc32_linmodel_ECIR1.5[i]
      Final_Summary_Effects[i,5] = TRUE
    }
  }
} else {
  colnames(Final_Summary_Effects) = c("Sample","Unit","log_ECIR1.5","log_SE_ECIR1.5","Confident_log_ECIR1.5?")
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$AREc32_logmodel_ECIR1.5[i]))){
      if(grepl("masked",Summary_results$AREc32_logmodel_ECIR1.5[i])){
        Final_Summary_Effects[i,3] = "masked by cytotoxicity"
        Final_Summary_Effects[i,4] = NA
        Final_Summary_Effects[i,5] = FALSE
      } else {
    ...
    } else if(is.na(as.numeric(Summary_results$AREc32_logmodel_confidence_ECIR1.5[i]))|as.numeric(Summary_results$AREc32_logmodel_confidence_ECIR1.5[i])<0.70){
      Final_Summary_Effects[i,3] = Summary_results$AREc32_logmodel_ECIR1.5[i]
      Final_Summary_Effects[i,4] = Summary_results$SE_AREc32_logmodel_ECIR1.5[i]
      Final_Summary_Effects[i,5] = FALSE
    } else {
...
if(Summary_model_cytotoxicity=="linear"){
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$AREc32_linmodel_IC10[i]))){
      Final_Summary_Cytotoxicity[i,1] = "no cytotoxicity"
      Final_Summary_Cytotoxicity[i,2] = NA
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else if(is.na(as.numeric(Summary_results$AREc32_linmodel_confidence_IC10[i]))|as.numeric(Summary_results$AREc32_linmodel_confidence_IC10[i])<0.70){
      Final_Summary_Cytotoxicity[i,1] = Summary_results$AREc32_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_AREc32_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else {
      Final_Summary_Cytotoxicity[i,1] = Summary_results$AREc32_linmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_AREc32_linmodel_IC10[i]
  ...
  for(i in 1:nrow(Summary_results)){
    if(is.na(as.numeric(Summary_results$AREc32_logmodel_IC10[i]))){
      Final_Summary_Cytotoxicity[i,1] = "no cytotoxicity"
      Final_Summary_Cytotoxicity[i,2] = NA
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else if(is.na(as.numeric(Summary_results$AREc32_logmodel_confidence_IC10[i]))|as.numeric(Summary_results$AREc32_logmodel_confidence_IC10[i])<0.70){
      Final_Summary_Cytotoxicity[i,1] = Summary_results$AREc32_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_AREc32_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,3] = FALSE
    } else {
      Final_Summary_Cytotoxicity[i,1] = Summary_results$AREc32_logmodel_IC10[i]
      Final_Summary_Cytotoxicity[i,2] = Summary_results$SE_AREc32_logmodel_IC10[i]
...
openxlsx::saveWorkbook(Output,paste0(Output_path,"/Summary_",project,"_AREc32_filled.xlsx"),overwrite = T)
